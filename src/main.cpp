...
void handleHome()
{
  ...
  s += "</ul>";
  s += "Go to <a href='home'>home page</a> to change values.";
  s += "</body></html>\n";
...
void serverForConfig()
{
  ...
  server.on("/savecfg", handleSaveConfig);
  server.on("/switch", handleSwitchConfig);
  server.on("/edititem", handleEditConfigItem);
  server.on("/getedititem", handleGetEditConfigItem);
  server.on("/inline", []() {
    server.send(200, "text/plain", "this is inline and works as well");
  });
