...
void handleSwitchConfig() {
  ...
  */
  for (uint8_t i = 0; i < server.args(); i++) {
    if(String(server.argName(i)).equals("param")) {
      if(String(server.arg(i)).equals("show_mgdl")) {
        cfg.show_mgdl = !cfg.show_mgdl;
        // Serial.print("show_mgdl = "); Serial.println(cfg.show_mgdl);
      }
      if(String(server.arg(i)).equals("sgv_only")) {
        cfg.sgv_only = !cfg.sgv_only;
      }
      if(String(server.arg(i)).equals("default_page")) {
        cfg.default_page++;
      ...
      if(String(server.arg(i)).equals("show_current_time")) {
        cfg.show_current_time = !cfg.show_current_time;
      }
      if(String(server.arg(i)).equals("show_COB_IOB")) {
        cfg.show_COB_IOB = !cfg.show_COB_IOB;
      }
      if(String(server.arg(i)).equals("snd_warning_at_startup")) {
        cfg.snd_warning_at_startup = !cfg.snd_warning_at_startup;
      }
      if(String(server.arg(i)).equals("snd_alarm_at_startup")) {
        cfg.snd_alarm_at_startup = !cfg.snd_alarm_at_startup;
      }
      if(String(server.arg(i)).equals("info_line")) {
        cfg.info_line++;
        if(cfg.info_line>2)
          cfg.info_line = 0;
      }
      if(String(server.arg(i)).equals("date_format")) {
        cfg.date_format++;
        if(cfg.date_format>1)
          cfg.date_format = 0;
      }
      if(String(server.arg(i)).equals("display_rotation")) {
      ...
      if(String(server.arg(i)).equals("invert_display")) {
        cfg.invert_display++;
      ...
      if(String(server.arg(i)).equals("temperature_unit")) {
        cfg.temperature_unit++;
        if(cfg.temperature_unit>3)
          cfg.temperature_unit = 1;
      }
      if(String(server.arg(i)).equals("dev_mode")) {
...
void handleEditConfigItem() {
  ...
  // char itemName[60];
  // char itemValue[250];
  if(String(server.argName(0)).equals("param")) {
    if(String(server.arg(0)).equals("userName")) {
      message += "M5Stack user name: \r\n";
      message += "<input type=\"text\" name=\"" + String(server.arg(0)) + "\" value=\"" + String(cfg.userName) + "\" size=\"20\" maxlength=\"32\"><br>\r\n";
    }
    if(String(server.arg(0)).equals("NSurl")) {
    ...
    if(String(server.arg(0)).equals("deviceName")) {
    ...
    if(String(server.arg(0)).equals("timeZoneDST")) {
    ...
    if(String(server.arg(0)).equals("restartAt")) {
    ...
    if(String(server.arg(0)).equals("alarmTiming")) {
    ...
    if(String(server.arg(0)).equals("dispColors")) {
    ...
    if(String(server.arg(0)).equals("sndAlarms")) {
    ...
    if(String(server.arg(0)).equals("brightness")) {
    ...
    if(String(server.arg(0)).equals("wlans")) {
...
void handleGetEditConfigItem() {
  ...
  for (uint8_t i = 0; i < server.args(); i++) {
    if(String(server.argName(i)).equals("userName")) {
      strncpy(cfg.userName, String(server.arg(i)).c_str(), 32);
    }
    if(String(server.argName(i)).equals("url")) {
      strncpy(cfg.url, String(server.arg(i)).c_str(), 128);
    }
    if(String(server.argName(i)).equals("token")) {
      strncpy(cfg.token, String(server.arg(i)).c_str(), 32);
    }
    if(String(server.argName(i)).equals("deviceName")) {
      strncpy(cfg.deviceName, String(server.arg(i)).c_str(), 32);
    }
    if(String(server.argName(i)).equals("timeZone")) {
      cfg.timeZone = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("dst")) {
      cfg.dst = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("restart_at_time")) {
      strncpy(cfg.restart_at_time, String(server.arg(i)).c_str(), 10);
    }
    if(String(server.argName(i)).equals("restart_at_logged_errors")) {
      cfg.restart_at_logged_errors = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("snooze_timeout")) {
      cfg.snooze_timeout = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("alarm_repeat")) {
      cfg.alarm_repeat = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("yellow_low")) {
      cfg.yellow_low = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("yellow_high")) {
      cfg.yellow_high = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("red_low")) {
      cfg.red_low = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("red_high")) {
      cfg.red_high = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("snd_warning")) {
      cfg.snd_warning = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("snd_warning_high")) {
      cfg.snd_warning_high = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("snd_alarm")) {
      cfg.snd_alarm = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("snd_alarm_high")) {
      cfg.snd_alarm_high = String(server.arg(i)).toFloat()/mult;
    }
    if(String(server.argName(i)).equals("snd_no_readings")) {
      cfg.snd_no_readings = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("warning_volume")) {
      cfg.warning_volume = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("alarm_volume")) {
      cfg.alarm_volume = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("brightness1")) {
      if(lcdBrightness==cfg.brightness1) {
        // changing selected brightness, so update it
        lcdBrightness = String(server.arg(i)).toInt();
        M5.Lcd.setBrightness(lcdBrightness);
      }
      cfg.brightness1 = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("brightness2")) {
      if(lcdBrightness==cfg.brightness2) {
        // changing selected brightness, so update it
        lcdBrightness = String(server.arg(i)).toInt();
        M5.Lcd.setBrightness(lcdBrightness);
      }
      cfg.brightness2 = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).equals("brightness3")) {
      if(lcdBrightness==cfg.brightness3) {
        // changing selected brightness, so update it
        lcdBrightness = String(server.arg(i)).toInt();
        M5.Lcd.setBrightness(lcdBrightness);
      }
      cfg.brightness3 = String(server.arg(i)).toInt();
    }
    if(String(server.argName(i)).startsWith("wlanssid")) {
      tmpStr = String(server.argName(i));
      tmpStr.remove(0, 8);
      int nr = tmpStr.toInt();
      message += "WLAN SSID [" + tmpStr + "] = " + String(server.arg(i)) + " (" + String(server.arg(i)).length() + ")<br />\r\n";
      strncpy(cfg.wlanssid[nr], String(server.arg(i)).c_str(), 32);
    }
    if(String(server.argName(i)).startsWith("wlanpass")) {
      tmpStr = String(server.argName(i));
      tmpStr.remove(0, 8);
      int nr = tmpStr.toInt();
      message += "WLAN PASS [" + tmpStr + "] = " + String(server.arg(i)) + " (" + String(server.arg(i)).length() + ")<br />\r\n";
      strncpy(cfg.wlanpass[nr], String(server.arg(i)).c_str(), 64);
    }
  }
  message += "</body>\r\n";
  message += "</html>\r\n";
...
void handleSaveConfig() {
  ...
  if(!SPIFFS.remove("/M5NS.BAK")) {
    Serial.println("Error removing M5NS.BAK");
  }
  srcFil = SPIFFS.open("/M5NS.INI", FILE_READ);
  if(!srcFil) {
    Serial.println("Error opening M5NS.INI for read");
  } else {
    dstFil = SPIFFS.open("/M5NS.BAK", FILE_WRITE);
    if(!dstFil) {
  ...
  if(!SPIFFS.remove("/M5NS.INI")) {
    Serial.println("Error removing M5NS.BAK");
  }
  dstFil = SPIFFS.open("/M5NS.INI", FILE_WRITE);
  if(!dstFil) {
